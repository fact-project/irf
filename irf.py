...
def histograms(predictions, showers, theta_square_cut, prediction_threshold):
    ...
    predictions['energy'] = predictions['MCorsikaEvtHeader.fTotalEnergy'].apply(
        np.log10)
    predictions['zenith'] = predictions['MCorsikaEvtHeader.fZd'].apply(
        np.rad2deg)

    hist_showers, x_edges, y_edges = np.histogram2d(
        showers.energy, showers.zenith, bins=(11, 4)
    )
    hist_data, x_edges, y_edges = np.histogram2d(
        predictions.energy, predictions.zenith, bins=(11, 4)
    )
...
def main(showers, predictions, prediction_threshold, theta_square_cut, impact):
    ...
    hist_showers, hist_data, x_edges, y_edges = histograms(
        predictions, showers, theta_square_cut, prediction_threshold
    )
    ...
    plt.imshow(
        ...
        origin='lower',
        cmap='plasma',
        extent=(showers.energy.min(), showers.energy.max(), 0, 60),
        aspect='auto'
    )
    ...
    plt.suptitle('Predictions Threshold > {} and Theta Square < {}'.format(
        prediction_threshold, theta_square_cut))
    plt.imshow(area.T, interpolation='nearest', origin='lower', cmap='plasma', extent=(
        showers.energy.min(), showers.energy.max(), 0, 60), aspect='auto')
    plt.xlabel(r'$\log_{10}(E /  \mathrm{GeV})$')
    plt.ylabel('Zenith Angle')
    plt.colorbar()
    ...
    for i in range(area.shape[1]):
        plt.errorbar(bin_center, area[:, i], xerr=bin_width / 2.0, linestyle='',
