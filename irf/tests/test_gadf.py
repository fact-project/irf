...
def test_edisp_irf_writing(predictions):
    t = oga.response.energy_dispersion_hdu(predictions, bins=5, theta_bins=1)
...
def test_aeff_irf_writing(showers, predictions, tmpdir):
    t = oga.response.effective_area_hdu(showers, predictions, bins=6)
